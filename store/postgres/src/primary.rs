...
impl<'a> Connection<'a> {
    ...
    pub fn allocate_site(
        ...
        if let Some(site) = queries::find_active_site(self.0.as_ref(), subgraph)? {
            return Ok(site);
        }
    ...
    pub fn assigned_node(&self, site: &Site) -> Result<Option<NodeId>, StoreError> {
        queries::assigned_node(self.0.as_ref(), site)
    }
    ...
    pub fn copy_site(&self, src: &Site, shard: Shard) -> Result<Site, StoreError> {
        if let Some(site) = queries::find_site_in_shard(self.0.as_ref(), &src.deployment, &shard)? {
            return Ok(site);
        }
